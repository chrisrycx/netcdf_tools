#!/usr/bin/env python3
"""
process_spires — Process SPIReS historical snow cover NetCDF4 files for the western US.
...
def _encoding_for_variable(da) -> dict:
    """
...
def process_day(tile_paths: list[Path], variable: str, output_path: Path) -> None:
    """
    ...
    time_enc = {"units": TIME_UNITS, "calendar": TIME_CALENDAR, "dtype": "int32"}
...
def assemble_output(daily_files: list[Path], output_path: Path, variable: str) -> None:
    ...
    try:
        ...
        raise ImportError(
            "xarray is required. Install with: pip install netcdf_tools[spires]"
        )

    log.info("Assembling %d daily files → %s", len(daily_files), output_path)

    ds = xr.open_mfdataset(sorted(daily_files), combine="by_coords", decode_times=True)
    ...
    ds.attrs.update({
        ...
        "spatial_extent": f"lon [{WEST_LON}, {EAST_LON}], lat [{SOUTH_LAT}, {NORTH_LAT}]",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--variable", default="snow_fraction",
                        help="Variable name to process (default: snow_fraction).")
    parser.add_argument("--start-date",
                        type=lambda s: datetime.strptime(s, "%Y%m%d").date(),
                        metavar="YYYYMMDD", help="First date to include (inclusive).")
    parser.add_argument("--end-date",
                        type=lambda s: datetime.strptime(s, "%Y%m%d").date(),
                        metavar="YYYYMMDD", help="Last date to include (inclusive).")
