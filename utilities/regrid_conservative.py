...
def regrid(input_file, target_file, output_file, variables=None, method="conservative"):
    ...
    # Build target grid dataset with just lat/lon
    ds_out = xr.Dataset({
        "lat": ds_target["lat"],
        "lon": ds_target["lon"],
    })
    ...
    # Separate variables: skip crs/time_str, only regrid vars with time/lat/lon coords
    ...
    # Restore attributes on regridded variables
    for v in regrid_vars:
        ds_regridded[v].attrs = ds_in[v].attrs

    # Add passthrough variables (no regridding needed) and preserve their attributes
    for v in passthrough_vars:
        ds_regridded[v] = ds_in[v]

    # Add skipped variables (crs, time_str) unchanged
    for v in skip_vars:
        ds_regridded[v] = ds_in[v]

    # Preserve global dataset attributes and add provenance
    ds_regridded.attrs = ds_in.attrs
    ds_regridded.attrs["regrid_method"] = f"xesmf {method}"
    ds_regridded.attrs["author"] = "Chris Cox"
